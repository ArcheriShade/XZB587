...
void pixel(int i, int j, char* str) {
	...
	case 8:
		...
		if (isGray())
			sprintf(str, "灰度值：%d", lpBitsInfo->bmiColors[*pixel].rgbBlue);
		else {
...
void Histogram() {
	...
	for (i = 0; i < 256; i++) {
		HGray[i] = HRad[i] = HGreen[i] = HBlue[i] = 0;
	}
	...
	case 8:
		if (isGray()) {
			for (i = 0; i < height; i++) {
				for (j = 0; j < width; j++) {
					pixel = lpBits + lineBytes*(height - 1 - i) + j;
					gray = lpBitsInfo->bmiColors[*pixel].rgbBlue;
					HGray[gray]++;
				}
			}
		}
			...
			for (i = 0; i < height; i++) {
				for (j = 0; j < width; j++) {
					...
					if (0 == j%2)
						...
						bv = bv&15;
					gray = lpBitsInfo->bmiColors[bv].rgbBlue;
					HGray[gray]++;
				}
			}
		}
